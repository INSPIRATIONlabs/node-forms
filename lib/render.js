...
var formItem = function(tag) {
  return function (name, field, iterator) {
    var w = field.widget;
    var classes=["field-type-"+field.widget.type].concat(field.classes(),w.classes);
    var attr = {'class':classes, 'id':'item-'+(field.id || name)};
    ...
    if(field.widget.type === 'fieldset') {//} || field.widget.type === 'multipleCheckbox' || field.widget.type === 'multipleRadio') {
      ...
      if(field.description) {
        desc = '<p class="description before">'+field.description+'</p>';
      }
      html = html.concat([
        '<fieldset>',
    ...
    } else if(field.widget.type === 'multipleCheckbox' || field.widget.type === 'multipleRadio') {
      html = html.concat([
        field.errorHTML(),
        '<p class="label">', field.labelText(name), '</p>',
        '<div class="choices">',
        field.widget.toHTML(name, field, iterator),
        '</div>'
      ]);
      if(field.description) {
        html.push('<p class="description">'+field.description+'</p>');
      }
    } else {
